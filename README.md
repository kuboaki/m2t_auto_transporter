...
ビルドの前に app.cfg を編集してください（どちらかしか使わない人は、あとでテンプレートを直しましょう）。

実機用にビルド・実行するひとは、 `app.cfg` を次のように編集してください。
...
箱庭用にビルド・実行するひとは、 `app.cfg` を次のように編集してください。
...
※ このような、実装時の処理方式を決める設計作業を実装方式と呼ぶ人もいます。モデル駆動の文献以外ではあまり語られていませんが、実装と設計の乖離を減らすには、設計の冒頭で実装方式を決定して、それから構造や振る舞いのモデルを作ることが大切です。
